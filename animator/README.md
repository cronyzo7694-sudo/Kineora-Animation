...
- **Frame ops (Part 07 §7.4 / Part 29.5)**: **F6** Insert Keyframe (copies previous content; no-op + honest toast when the frame is already a content keyframe; on a blank keyframe it restores the pre-blank content), **F7** Insert Blank Keyframe, **Shift+F6** Clear Keyframe, **F5** Insert Frame (extend the hold — later keyframes shift right), **Shift+F5** Delete Frame (remove + shift later left) — all undoable Rust commands. **Ctrl+Z / Ctrl+Shift+Z / Ctrl+Y** undo/redo globally (Part 29.2). Home/End transport. Frame ops are blocked on locked layers (Part 20.2 "not editable") with a clear toast; allowed on hidden layers.
- **Keyframe dragging** (Part 07 §7.4.9 / F-07-12 E1/E2): drag a keyframe dot to move it (one command on release), **Alt-drag** to duplicate it, plain click selects the cell, Esc cancels, zero-delta/collision = no command. Moving a keyframe relocates just that record (neighbors don't shift).
- **Timeline zoom** (F-07-03 ruler zoom — view state, never persisted): **− / +** buttons scale cell width 50%–400% with adaptive ruler numbering (sparser when zoomed out, denser when zoomed in). Playhead/cells/dots/handles all remap exactly; the playhead frame and the document never change.
- **Transport + loop** (F-07-04 / Part 07 §7.1.5 / C-08): **`.`/`,`** step a frame, **Alt+`,`/Alt+`.`** keyframe-hop, ⏮/⏭ first/last, ◎ center-playhead, and a **⟳ Loop** toggle — loop ON wraps 1..duration, loop OFF stops at the last frame (default ON). All view state (no undo).
- **Frame range selection + clipboard/sequence ops** (Part 07 §7.4.6–10 / F-07-12/13): click selects a frame, **drag selects a contiguous range** (single layer), Shift/Ctrl+click toggles. **Copy / Cut / Paste / Reverse / Remove** buttons operate on the selection through undoable Rust commands — the frame clipboard is engine session state (never persisted/undone); paste preserves relative offsets and overwrites collisions; Remove leaves a gap; Reverse flips the keyframe order. Locked layers block all mutating ops (Copy stays read-only); no-ops create no commands.
- **Classic tween + easing** (Part 09.2 / Part 08 §8.0 / Part 09.4): interpolation is now **explicit** — frame-by-frame keyframes HOLD, and a **classic tween span** (blue cells + ▶ end arrow) interpolates between two keyframes holding the same object. **~ Tween** / **✕ Tween** buttons + an **ease slider** (−100..+100, one command on release). Interpolation covers x/y/scale and rotation (shortest path). Frame ops keep tween spans consistent (shift/drop with their keyframes); spans + ease survive save/load; tweened frames export interpolated. Motion tween (per-property, needs symbols) and shape tween (needs shapes) are later units.
- **Frame sequences, exposure & labels** (Part 07 §7.4.8–12 / §7.2): dragging a keyframe dot moves it **with its held span** (exposure preserved, overwrite prompt on collision); a **hollow-rect span-edge handle** extends/shortens exposure (min 1 frame, Esc cancels); **⧉ Dup / ▣ Keys / □ Blanks** duplicate a range or convert it to (blank) keyframes; **frame labels** (red flag) set/clear on keyframes and persist — all undoable Rust commands, none of the timeline visuals leak into export.
- **Symbols + Library** (Part 11/12): **F8** converts the selection into a symbol (name/type/9-point registration grid); **Ctrl+F8** creates a symbol (clear toast + highlighted Library row; edit mode is a later unit). The Library panel lists symbols (name/type/use-count) with create / ID-safe rename / delete (in-use → prompt + break-apart) / drag-out-to-place / drag-onto-instance-to-swap, and honest states for engine-unattached vs out-of-date vs empty. **Nested evaluation** (Part 11.8): graphic instances sync to the parent clock, movie clips run a free clock; empty symbols stay selectable via a deterministic marker that never leaks into export; clicking/marquee select the instance, not its inner art. Selecting an instance reveals **Loop / Play Once / Single Frame / First frame** and a **Swap** control in Properties (all undoable, persisted). Everything round-trips save/load; convert is blocked on locked layers; SVG export flattens nested content cleanly.
- **Playback loops 1..derived duration** (`timeline_duration` = max keyframe frame, min 1) — the playhead never travels past the animation range (engineering REQ-TIM-004). The timeline viewport is a horizontally scrollable, **auto-extending** strip (no artificial frame cap — navigation past frame 60 grows the viewport) so you can author frames beyond the current derived duration.
- **Locked layers** (Part 20.2 "not editable"): all three frame ops are blocked and the Key/Blank/Clear buttons are **disabled with a "🔒 layer locked" hint** when the active layer is locked. **Undo/redo stay global** (reversing history is not "editing"; the lock toggle itself is an undoable command, F-03-15 TS-12).
...
- **Panel/layout system** (C-06/C-08/C-09): one reusable splitter (horizontal + vertical, 6px, live drag, Esc/pointercancel/blur cancel, min/max clamp) drives the whole workspace. The right dock is a **bounded, overflow-safe region** — Properties (flex, ≥320px) + Library + Dev sizes come from a generic `distribute()` engine with **sum-aware splitter clamps**, so no panel can reach zero or be pushed off-screen (the region scrolls as the deliberate last resort). One **visibility registry** covers Layers/Properties/Library/**Timeline**/**Dev** (Timeline via **Ctrl+Alt+T**; height preserved across hide/show). Layers width 140–480, Properties width 240–520, Timeline height 96px..60% viewport. Sizes persist to app prefs (never the document) and **⟲ Reset Workspace** restores defaults. Resizing/hide-show is pure view state — it never touches the engine, undo, frame zoom, or export.
