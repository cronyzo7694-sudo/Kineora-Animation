...
describe('A6.3 modes and execution gates', () => {
  ...
  it('make a ball previews a supported oval and APPLY uses one A5 transaction', async () => {
    const h = harness()
    const preview = await h.orchestrator.generate({ documentId: 1, userRequest: 'make a ball', mode: 'preview' })
    expect(preview.status).toBe('awaitingApproval')
    expect(preview.plan?.actions[0]?.action).toBe('shape.create')
    expect(preview.plan?.actions[0]?.params.shape).toBe('oval')
    expect(h.run).not.toHaveBeenCalled()
    const applied = await h.orchestrator.approve(1)
    expect(h.run).toHaveBeenCalledTimes(1)
    expect(applied).toMatchObject({ ok: true, status: 'completed', verification: { verdict: 'pass' } })
  })
