...
impl Session {
    ...
    // ————— SYS-16 folder facades (F-20-05) —————
    // [REPAIR — INT-AID-005, ownership stays AI-C/SYS-16]: these three
    // facades were LOST from commit 9128ad9 — its session.rs hunk was
    // mangled into a duplicated file tail (see INT-AID-004) while the
    // command structs (CMD-LAYER-PARENT / CMD-FOLDER-COLLAPSE), model
    // helpers (is_folder/layer_descendants/layer_is_ancestor), wasm bridges
    // (wasm.rs:807-828), UI client calls and tests/layers.rs ALL landed.
    // Reconstructed VERBATIM against those artifacts of AI-C's own commit —
    // no semantics invented by AI-D. Flagged for AI-C review.

    /// F-20-05: create a layer FOLDER (organizational — no frames, never a
    /// draw target; `draw_rect` blocks on folders). Placement/undo pattern
    /// mirrors `create_layer` (insert above active, one undo step).
    pub fn create_folder(&mut self) -> Option<usize> {
        let scene = self.active_scene;
        let count = self.doc.scene(scene)?.layers.len();
        let index = (self.active_layer + 1).min(count);
        ...
        self.history.execute(&mut self.doc, Box::new(cmd));
        self.active_layer = index;
        self.log(&format!("layer:createFolder@{index}"));
        Some(index)
    }
    ...
    /// F-20-05: nest `child` under folder `parent` (None = un-nest to root).
    /// ONE undo step (CMD-LAYER-PARENT). Refuses: missing indices · a
    /// non-folder parent · any cycle (a folder can never nest under itself
    /// or one of its own descendants) · a no-op re-parent.
    pub fn set_layer_parent(&mut self, child: usize, parent: Option<usize>) -> bool {
        let scene = self.active_scene;
        let Some(sc) = self.doc.scene(scene) else {
            return false;
        };
        let Some(c) = sc.layers.get(child) else {
            return false;
        };
        let child_id = c.id;
        let before = c.parent_id;
        let after = match parent {
            None => None,
            Some(pi) => {
                let Some(p) = sc.layers.get(pi) else {
                    return false;
                };
                if !p.is_folder() {
                    return false; // only folders can hold children
                }
                if self.doc.layer_is_ancestor(scene, child_id, p.id) {
                    return false; // cycle (includes parent == child)
                }
                Some(p.id)
            }
        };
        ...
        self.history.execute(&mut self.doc, Box::new(cmd));
        self.log(&format!("layer:parent@{child}"));
        true
    }

    /// F-20-05: triangle expand/collapse (undoable, persisted —
    /// CMD-FOLDER-COLLAPSE). Only folders collapse; no-op toggles refuse.
    pub fn set_folder_collapsed(&mut self, index: usize, collapsed: bool) -> bool {
        let scene = self.active_scene;
        let Some(l) = self
            .doc
        ...
        self.history.execute(&mut self.doc, Box::new(cmd));
        self.log(&format!("layer:collapse@{index}={collapsed}"));
        true
    }
