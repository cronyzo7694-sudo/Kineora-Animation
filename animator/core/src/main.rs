...
fn main() {
    ...
    println!("[1] create document: {}x{} @{}fps", s.doc.settings.width, s.doc.settings.height, s.doc.settings.fps);
    ...
    println!("[4] keyframe@10 + move → frame10 rects {:?}", s.evaluate(10));
    ...
    println!("[8] export SVG (frame10) → /tmp/out.svg ({} bytes)", svg.len());

    s.save(std::path::Path::new("/tmp/out.json")).expect("save");
    let loaded = Session::load(std::path::Path::new("/tmp/out.json")).expect("load");
    println!("[9] save→load round-trip: nodes={}, scenes={}", loaded.doc.nodes.len(), loaded.doc.scenes.len());
    println!("[10] event log:\n{}", s.event_log.iter().map(|e| format!("  - {e}")).collect::<Vec<_>>().join("\n"));
}
