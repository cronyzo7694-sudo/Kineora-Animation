...
- **Symbols + Library** (Part 11/12): **F8** converts the selection into a symbol (name/type/9-point registration grid) — one instance replaces it, the registration point becomes the symbol's origin. **Ctrl+F8** creates an empty symbol. The Library panel lists symbols (name/type/use-count), with create / ID-safe rename / delete (in-use → prompt + break-apart) / drag-out-to-place / drag-onto-instance-to-swap. **Nested evaluation** (Part 11.8): graphic instances sync to the parent clock (Loop / Play Once / Single Frame + first frame), movie clips run a free clock; click/marquee select the instance, not its inner art; SVG export flattens nested content cleanly. Everything round-trips save/load and is undoable.
