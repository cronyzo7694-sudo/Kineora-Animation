...
fn draw_oval_creates_parametric_oval_node_and_selects_it() {
    ...
    assert_eq!(
        s.selection,
        vec![id],
        "new shape is selected on commit (T2B.5 §8)"
    );
}
...
fn oval_click_hit_test_is_exact_never_the_bounding_box() {
    ...
    assert!(
        !s.select_at(10.0, 10.0),
        "((10-50)/50)²·2 = 1.28 > 1 — still air"
    );
}
...
fn rotated_oval_hit_test_uses_the_true_ellipse() {
    ...
    assert!(
        s.select_at(100.0, 140.0),
        "long-axis rim point hits after rotation"
    );
    // along the (now horizontal) short axis: 60 > 50 → air for the rotated ellipse
    assert!(
        !s.select_at(160.0, 50.0),
        "short-axis air does not hit after rotation"
    );
}
...
fn marquee_touching_only_bounding_box_air_selects_nothing() {
    ...
    assert!(
        ...
        "contact with corner air is not contact"
    );
}
...
fn marquee_that_touches_the_oval_selects_it() {
    ...
    assert_eq!(
        s.selection,
        vec![id],
        "the box reaches the rim at (50, 0..3)"
    );
}
...
fn rotated_oval_exports_with_rotate_around_centre() {
    ...
    assert!(
        svg.contains(
            r##"<ellipse cx="100" cy="50" rx="100" ry="50" fill="#ff8800" transform="rotate(90 100 50)"/>"##
        ),
    ...
    match back {
        Node::Rect { shape, .. } => {
            assert_eq!(
                shape,
                ShapeKind::Rect,
                "missing shape field = plain rectangle"
            )
        }
